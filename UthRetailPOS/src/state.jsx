...
const API   = "/api/state";
const LIMIT = 20;
const EMPTY = { st_code: "", st_nameen: "", st_namereg: "", st_countryid: "", st_active: true };

const ALL_COLS = [
  { key: "st_code",      label: "Code" },
  { key: "st_nameen",    label: "Name (English)" },
  { key: "st_namereg",   label: "Name (Regional)" },
  { key: "st_countryid", label: "Country" },
...
export default function StateManager() {
  ...
  const [sortBy, setSortBy]           = useState("st_nameen");
  ...
  useEffect(() => {
    fetch("/api/country").then(r => r.json()).then(d => setCountries(d.rows || [])).catch(() => {});
  }, []);
  ...
  const openCreate = async () => {
    setFormData({ ...EMPTY, st_code: "Loading..." });
    setEditRecid(null);
    setView("form");
    try {
      const res  = await fetch(`${API}/next-code`);
      const data = await res.json();
      if (data.success) setFormData(f => ({ ...f, st_code: data.code }));
    } catch {
      setFormData(f => ({ ...f, st_code: "ST-AUTO" }));
    }
  };
  ...
  const openEdit = async (recid) => {
    try {
      ...
      setFormData({
        st_code:      s.st_code,
        st_nameen:    s.st_nameen,
        st_namereg:   s.st_namereg || "",
        st_countryid: s.st_countryid || "",
        st_active:    s.st_active,
      });
      setEditRecid(recid);
      setView("form");
    } catch (e) {
  ...
  const handleSave = async () => {
    if (!formData.st_nameen.trim()) { showToast("State Name (English) is required.", "error"); return; }
    setSaving(true);
    try {
      const body = JSON.stringify({
        st_nameen:    formData.st_nameen.trim(),
        st_namereg:   formData.st_namereg.trim(),
        st_countryid: formData.st_countryid || null,
      ...
      showToast(editRecid ? "State updated successfully." : "State created successfully.");
      setView("list");
      setEditRecid(null);
    } catch (e) {
    ...
    <div className="cm-root">
      <div className="cm-page">
        ...
        <div className="cm-subheader">
          <span className="cm-subheader-title">State</span>
          <div style={{ flex: 1 }} />
          {view === "form" ? (
            <>
        ...
        <div className="cm-card">
          ...
          {view === "form" && (
            <>
              <div className="cm-card-header">STATE – {editRecid ? "EDIT RECORD" : "SINGLE RECORD VIEW"}</div>
              <div className="cm-form-grid">
                <div className="cm-field">
                  <label>State Code</label>
                  <input type="text" value={formData.st_code} disabled />
                </div>
                <div className="cm-field">
                  <label>State Name (English) *</label>
                  <input type="text" value={formData.st_nameen} placeholder="e.g. Maharashtra"
                    onChange={e => setField("st_nameen", e.target.value)} />
                </div>
                <div className="cm-field">
                  <label>State Name (Regional)</label>
                  <input type="text" value={formData.st_namereg} placeholder="e.g. महाराष्ट्र"
                    onChange={e => setField("st_namereg", e.target.value)} />
                </div>
                <div className="cm-field">
                  <label>Country</label>
                  <select value={formData.st_countryid} onChange={e => setField("st_countryid", e.target.value)}
                    ...
                    {countries.map(c => <option key={c.con_recid} value={c.con_recid}>{c.con_nameen}</option>)}
                  </select>
                </div>
                <div className="cm-field">
                  <label>Active</label>
          ...
          {view === "list" && (
            <>
              <div className="cm-card-header">STATE – LIST VIEW</div>
                ...
                <table className="cm-table">
                  <thead>
                    <tr>
                      ...
                      {visibleCols.st_code      && <th onClick={() => handleSort("st_code")} style={{ cursor: "pointer" }}>Code{sortIcon("st_code")}</th>}
                      {visibleCols.st_nameen    && <th onClick={() => handleSort("st_nameen")} style={{ cursor: "pointer" }}>Name (English){sortIcon("st_nameen")}</th>}
                      {visibleCols.st_namereg   && <th>Name (Regional)</th>}
                      {visibleCols.st_countryid && <th>Country</th>}
                    ...
                    {loading ? (
                      ...
                      <tr><td colSpan={colSpan}>
                        <div className="cm-empty">
                          ...
                          <div>{search ? "No matching states." : "No states yet. Click + Create New."}</div>
                        </div>
                      </td></tr>
                    ) : rows.map(r => (
                      <tr key={r.st_recid}>
                        ...
                        {visibleCols.st_code      && <td><strong>{r.st_code}</strong></td>}
                        {visibleCols.st_nameen    && <td>{r.st_nameen}</td>}
                        {visibleCols.st_namereg   && <td style={{ color: "#666" }}>{r.st_namereg || "—"}</td>}
                        {visibleCols.st_countryid && <td style={{ color: "#666" }}>{r.st_countryid || "—"}</td>}
        ...
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.4)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:999 }}>
          <div style={{ background:"#fff", borderRadius:12, padding:"28px 32px", minWidth:320, textAlign:"center", boxShadow:"0 8px 32px rgba(0,0,0,0.2)" }}>
            ...
            <p style={{ color:"#6b7280", marginBottom:20, fontSize:13 }}>
              {confirmDelete === "bulk" ? `Permanently delete ${selected.length} selected states?` : "Permanently delete this state?"}
            </p>
