...
const API   = "/api/banks";
const LIMIT = 20;
const EMPTY = { bk_code: "", bk_nameen: "", bk_namereg: "", bk_companyid: "", bk_active: true };

const ALL_COLS = [
  { key: "bk_code",      label: "Code" },
  { key: "bk_nameen",    label: "Name (English)" },
  { key: "bk_namereg",   label: "Name (Regional)" },
  { key: "bk_companyid", label: "Company" },
  { key: "bk_active",    label: "Active" },
...
export default function BanksManager() {
  ...
  const [sortBy, setSortBy]           = useState("bk_nameen");
  ...
  const openCreate = async () => {
    setFormData({ ...EMPTY, bk_code: "Loading..." });
    setEditRecid(null);
    setView("form");
    try {
      const res  = await fetch(`${API}/next-code`);
      const data = await res.json();
      if (data.success) setFormData(f => ({ ...f, bk_code: data.code }));
    } catch {
      setFormData(f => ({ ...f, bk_code: "BK-AUTO" }));
    }
  };
  ...
  const openEdit = async (recid) => {
    try {
      ...
      setFormData({
        bk_code:      b.bk_code,
        bk_nameen:    b.bk_nameen,
        bk_namereg:   b.bk_namereg || "",
        bk_companyid: b.bk_companyid || "",
        bk_active:    b.bk_active,
      });
      setEditRecid(recid);
      setView("form");
    } catch (e) {
  ...
  const handleSave = async () => {
    if (!formData.bk_nameen.trim()) { showToast("Bank Name (English) is required.", "error"); return; }
    setSaving(true);
    try {
      const body = JSON.stringify({
        bk_nameen:    formData.bk_nameen.trim(),
        bk_namereg:   formData.bk_namereg.trim(),
        bk_companyid: formData.bk_companyid.trim(),
        bk_active:    formData.bk_active,
      });
      ...
      showToast(editRecid ? "Bank updated successfully." : "Bank created successfully.");
      setView("list");
      setEditRecid(null);
    } catch (e) {
  ...
  const allChecked = rows.length > 0 && rows.every(r => selected.includes(r.bk_recid));
  const toggleAll  = (e) => setSelected(e.target.checked ? rows.map(r => r.bk_recid) : []);
    ...
    <div className="cm-root">
      <div className="cm-page">
        <div className="cm-subheader">
          <span className="cm-subheader-title">Banks</span>
          <div style={{ flex: 1 }} />
          {view === "form" ? (
            <>
        ...
        <div className="cm-card">
          {view === "form" && (
            <>
              <div className="cm-card-header">BANKS – {editRecid ? "EDIT RECORD" : "SINGLE RECORD VIEW"}</div>
              <div className="cm-form-grid">
                <div className="cm-field">
                  <label>Bank Code</label>
                  <input type="text" value={formData.bk_code} disabled />
                </div>
                <div className="cm-field">
                  <label>Bank Name (English) *</label>
                  <input type="text" value={formData.bk_nameen} placeholder="e.g. State Bank of India"
                    onChange={e => setField("bk_nameen", e.target.value)} />
                </div>
                <div className="cm-field">
                  <label>Bank Name (Regional)</label>
                  <input type="text" value={formData.bk_namereg} placeholder="Regional name"
                    onChange={e => setField("bk_namereg", e.target.value)} />
                </div>
                <div className="cm-field">
                  <label>Company</label>
                  <input type="text" value={formData.bk_companyid} placeholder="Company name / ID"
                    onChange={e => setField("bk_companyid", e.target.value)} />
                </div>
                <div className="cm-field">
                  <label>Active</label>
                  <label className="cm-toggle" style={{ marginTop: 6 }}>
                    <input type="checkbox" checked={formData.bk_active}
                      onChange={e => setField("bk_active", e.target.checked)} />
          ...
          {view === "list" && (
            <>
              <div className="cm-card-header">BANKS – LIST VIEW</div>
                ...
                <table className="cm-table">
                  <thead>
                    <tr>
                      ...
                      {visibleCols.bk_code      && <th onClick={() => handleSort("bk_code")} style={{ cursor: "pointer" }}>Code{sortIcon("bk_code")}</th>}
                      {visibleCols.bk_nameen    && <th onClick={() => handleSort("bk_nameen")} style={{ cursor: "pointer" }}>Name (English){sortIcon("bk_nameen")}</th>}
                      {visibleCols.bk_namereg   && <th>Name (Regional)</th>}
                      {visibleCols.bk_companyid && <th>Company</th>}
                      {visibleCols.bk_active    && <th className="center">Active</th>}
                    ...
                    {loading ? (
                      ...
                      <tr><td colSpan={colSpan}>
                        <div className="cm-empty">
                          ...
                          <div>{search ? "No matching banks." : "No banks yet. Click + Create New."}</div>
                        </div>
                      </td></tr>
                    ) : rows.map(r => (
                      <tr key={r.bk_recid}>
                        <td><input type="checkbox" checked={selected.includes(r.bk_recid)}
                          onChange={e => toggleRow(r.bk_recid, e.target.checked)} /></td>
                        {visibleCols.bk_code      && <td><strong>{r.bk_code}</strong></td>}
                        {visibleCols.bk_nameen    && <td>{r.bk_nameen}</td>}
                        {visibleCols.bk_namereg   && <td style={{ color: "#666" }}>{r.bk_namereg || "—"}</td>}
                        {visibleCols.bk_companyid && <td style={{ color: "#666" }}>{r.bk_companyid || "—"}</td>}
                        {visibleCols.bk_active    && (
                          <td className="center">
                            <label className="cm-toggle" title="Toggle active">
                              <input type="checkbox" checked={r.bk_active}
                                onChange={e => toggleActive(r.bk_recid, e)} />
                              <span className="cm-toggle-slider" />
                            </label>
                          </td>
                        )}
                        {visibleCols.actions && (
                          <td className="center">
                            <button className="cm-btn-edit" onClick={() => openEdit(r.bk_recid)}>Edit</button>
                            <button className="cm-btn-danger" onClick={() => setConfirmDelete(r.bk_recid)}>Delete</button>
                          </td>
                        )}
                      </tr>
                    ))}
        ...
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.4)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:999 }}>
          <div style={{ background:"#fff", borderRadius:12, padding:"28px 32px", minWidth:320, textAlign:"center", boxShadow:"0 8px 32px rgba(0,0,0,0.2)" }}>
            ...
            <p style={{ color:"#6b7280", marginBottom:20, fontSize:13 }}>
              {confirmDelete === "bulk" ? `Permanently delete ${selected.length} selected banks?` : "Permanently delete this bank?"}
            </p>
